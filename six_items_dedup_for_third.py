...
def adjust_for_perm_pd(all_pd, perm):
	adj_pd = []
	running_ind = 0
	for pd_allocs in all_pd:
		running_ind += 1
		if running_ind % 10 == 0:
			print("PD {}".format(running_ind))
		if pd_allocs is None:
			adj_pd.append(None)
		else:
			new_dict = {}
			for alloc_int, rel_pd_int in pd_allocs.items():
				rel_pd = int_to_set(rel_pd_int)
				alloc = int_to_tuple[alloc_int]
				new_alloc = [0] * 6
				for i in range(6):
					new_alloc[perm[i]] = alloc[i]
				new_alloc_int = tuple_to_int[tuple(new_alloc)]
				new_set = set([])
				for elem_int in rel_pd:
					elem = int_to_tuple[elem_int]
					new_elem = [0] * 6
					for i in range(6):
						new_elem[perm[i]] = elem[i]
					new_set.add(tuple_to_int[tuple(new_elem)])

				new_dict[new_alloc_int] = set_to_int(new_set)
			adj_pd.append(new_dict)

	return adj_pd

def adjust_for_perm_efx(efx_eqconf, perm):
	adj_efx = []
	running_ind = 0
	for allocs in efx_eqconf:
		running_ind += 1
		if running_ind % 10 == 0:
			print("EFX {}".format(running_ind))
		if allocs is None:
			adj_efx.append(None)
		else:
			new_allocs = set([])
			for alloc in allocs:
				...
				for i in range(6):
					new_alloc[perm[i]] = alloc[i]
				new_allocs.add(tuple_to_int[tuple(new_alloc)])
			adj_efx.append(new_allocs)
...
for items in range(6,7):
	...
	for perm in permutations([0,1,2,3,4,5]):
		adj_pd = adjust_for_perm_pd(pd_for_efx_by_eqconf_base, perm)
		adj_efx = adjust_for_perm_efx(efx_by_eqconf_base, perm)
		print("Adjusted pd and efx for perm {}".format(perm))
		for i1 in range(len(adj_pd)):
			if adj_pd[i1] is not None:
				running_ind += 1
				...
				for alloc in iebe:
					iebe_pd.append(adj_pd[i1][alloc])

				final_tuple = tuple([set_to_int(iebe)] + iebe_pd)
				dedup_dict[final_tuple] = 1
		print("deduped {} vs total {}".format(len(dedup_dict), running_ind))
